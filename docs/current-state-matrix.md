...
Latest spatial hardening notes: `docs/spatial-intelligence-v2.md`.
Live spatial acceptance tests: `docs/spatial-live-test-plan.md`.
...
| Spatial grounding collapses to approximate bounds, active-object fallback, or centroid guesses | `patched` | Spatial v2 now uses deterministic AABB geometry in `vectra/tools/spatial.py`, true world bounds in `vectra/agent/observation.py`, strict spatial tools in `vectra/tools/spatial_tools.py`, tuple/list coordinate preservation in `agent_runtime/director/resolver.py`, and same-batch object refs in `agent_runtime/director/loop.py`. This fixes grounding/placement mechanics when the Director calls the right tools. |
...
3. Run the spatial live tests in `docs/spatial-live-test-plan.md` before trusting broad scene prompts.
4. Run the updated audit suite, especially the docs ceiling prompt under `vectra-dev`, with provider/model chain recorded per run.
5. Confirm that structural partial progress improves scene completion instead of only rewarding object churn.
6. Check whether batch pressure improves scene composition or just changes the failure shape.
7. Build the missing SIG/EG, visual-feedback, objective-completion, LangGraph/sub-agent, imported-model, composite-builder, and prompt-obligation layers before judging Vectra as a real scene-production agent.
